...
SimpleForm.setup do |config|
  ...
  config.wrappers :vertical_collection,
    ...
    b.use :full_error,
          wrap_with: {
            tag: 'p',
            class: 'block mt-2 text-red-500 text-xs italic'
          }
    b.use :hint,
          wrap_with: {
            tag: 'p',
  ...
  config.wrappers :vertical_multi_select,
    ...
    b.wrapper tag: 'div', class: 'inline-flex space-x-1' do |ba|
      ...
      ba.use :input,
             class:
               'flex w-auto w-auto shadow appearance-none border border-gray-300 rounded w-full p-2 bg-white focus:outline-none focus:border-blue-500 text-gray-400 leading-4 transition-colors duration-200 ease-in-out'
    end
    b.use :full_error,
          wrap_with: {
            tag: 'p',
